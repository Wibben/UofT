...
int application::run(setup_callback_fn initial_setup_user_callback,
    ...
    key_callback_fn key_release_user_callback)
{
  ...
  key_release_callback = key_release_user_callback;
...
void application::register_default_events_callbacks(ezgl::application *application)
{
  ...
  // Connect the release_key function to keyboard releases in MainWindow
  g_signal_connect(window, "key_release_event", G_CALLBACK(release_key), application);
...
void application::register_default_buttons_callbacks(ezgl::application *application)
{
  ...
  // Connect press_find_in_range function to the Find in Range button
  GObject *pois_in_range_button = application->get_object("POIsInRangeButton");
  g_signal_connect(pois_in_range_button, "clicked", G_CALLBACK(press_pois_in_range), application);
  
  // Connect press_load_map function to the Load Map button
  GObject *load_map_button = application->get_object("LoadMapButton");
  g_signal_connect(load_map_button, "clicked", G_CALLBACK(press_load_map), application);
  
  // Connect press_directions function to the Directions button
  GObject *directions_button = application->get_object("DirectionsButton");
  g_signal_connect(directions_button, "clicked", G_CALLBACK(press_directions_from), application);
  
  // Connect press_night_mode function to the Night Mode button
  GObject *night_mode_toggle = application->get_object("NightModeToggle");
  g_signal_connect(night_mode_toggle, "clicked", G_CALLBACK(press_night_mode), application);
}
