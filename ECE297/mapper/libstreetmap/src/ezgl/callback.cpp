...
gboolean release_key(GtkWidget *, GdkEventKey *event, gpointer data)
{
  auto application = static_cast<ezgl::application *>(data);
  
  // Call the user-defined key press callback if defined
  if(application->key_release_callback != nullptr) {
    // see: https://developer.gnome.org/gdk3/stable/gdk3-Keyboard-Handling.html
    application->key_release_callback(application, event, gdk_keyval_name(event->keyval));
  }
