...
figureHandle = figure('NumberTitle','off','Name','Tracking' );
...
% Create title
title('Lab2-4-4','FontWeight','bold','FontSize',15);
...
while time(end)<stopTime
    ...
    ref(count) = fscanf(serialObject,'%f')/1000;
...
hline3 = plot(time,ref,'k','Linewidth',1);
hline4 = plot(time,vel_ave,'m','Linewidth',2);
xlabel('Time [s]','FontWeight','bold','FontSize',16)
legend([hline1,hline2,hline3,hline4],'Position','Velocity','Reference Velocity','Average Velocity');
hold off
