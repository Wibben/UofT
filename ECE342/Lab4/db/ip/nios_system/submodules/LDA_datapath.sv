...
(
	...
	logic steep;
	logic[8:0] x0,x1,x,x0_steep,x1_steep,x0_saved,x1_saved;
	logic[8:0] y0,y1,y,y0_steep,y1_steep,y0_saved,y1_saved;
	logic[8:0] deltax,deltay;
	logic[2:0] colour_saved;
	
	// Compute steepness and preprocessing inputs (swaps)
	always_comb begin
		deltax = (x1_saved>x0_saved) ? x1_saved-x0_saved:x0_saved-x1_saved;
		deltay = (y1_saved>y0_saved) ? y1_saved-y0_saved:y0_saved-y1_saved;
	...
	always_ff@(posedge clk) begin
		if(reset) begin // Reset on startup
			...
			y <= 9'd0;
			error <= 10'd0;
		end else if(save_coord) begin // Save x0, x1, y0, y1 from curtrent inputs
			x0_saved <= i_x0;
			x1_saved <= i_x1;
			y0_saved <= i_y0;
			y1_saved <= i_y1;
			colour_saved <= i_colour;
